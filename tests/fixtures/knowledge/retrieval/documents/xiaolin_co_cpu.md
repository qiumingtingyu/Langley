...
2. CPU = 数据通路 + 控制部件
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240428133932.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240428151837.png)

- 指令周期最多有 4 种机器周期：**取指周期、间址周期、执行周期、中断周期**（都有访问主存的操作）
- 分别对应标志触发器：**FE、IND、EX、INT**（“1”表示有效，如 1-->FE 表示有取值周期）
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240428152035.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240428160538.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240429162059.png)
...
- **数据流向**：假设程序断点存入堆栈中，并用 SP 指示栈顶指针，而且进栈操作是先修改指针，后存入数据；出栈操作是先删除数据，后修改指针
	...
	4. CU（**中断服务程序的入口**）--->PC
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240429162754.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240429184829.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240429191754.png)
#### （2）CU 的输入信号来源
- 经指令编译器译码产生的指令信息
- 时序系统产生的机器周期信号和节拍信号
- 来自执行单元的反馈信号，即标志
...
#### （5）设计步骤
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240429205441.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240429200221.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240429202112.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240429202435.png)
...
- **优点**：可进一步缩短微指令字长
- **缺点**：削弱了微指令的并行能力
![](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240429202952.png)
...
- **优点**：微程序短，执行速度快
- **缺点**：微指令长，编写微程序比较麻烦
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240429203656.png)
...
- **缺点**：微程序长，执行速度慢，工作效率低
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240429203930.png)
...
#### （9）设计步骤
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240429205320.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240501220002.png)
...
#### （4）流水线的表示方法
- 采用时空图描述流水线的执行情况
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240501220833.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240501221430.png)
- **注意**：
...
- 最大加速比： $S_{max}=k$
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240501222028.png)


##### 3）流水线的效率（E）
- **效率**：流水线的设备利用率，在时空图上，表示为**完成 n 个任务占用的时空区有效面积**与 **n 个任务所用的时间与 k 个流水段所围成的时空区总面积**之比
- $E=\frac{T_0}{kT_k}$
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240501223026.png)
...
- **数据通路不包含生成控制信号的控制部件**
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240501223439.png)

#### （2）流水线的控制信号
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240501223611.png)
- **流水寄存器保存的信息**
	- 后面流水段要用到的**所有数据信息**
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240501224958.png)
**解决方法**：
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240502153843.png)
**解决方法**：
- **延迟执行相关指令**：把遇到数据相关的指令及其后续指令都暂停一至几个时钟周期，直至数据相关问题消失后再继续执行，可分为**硬件阻塞（stall）** 和**软件插入空操作 nop**
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240502154533.png)

- **转发（旁路）技术**：设置相关转发通路，不等前一条指令把计算结果写回寄存器，下一条指令也不再从寄存器读，而将数据通路中生成的中间数据**直接转发到 ALU 的输入端**
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240502154556.png)

- **load-use 数据冒险处理**：load 指令与紧邻的运算类指令存在数据相关问题，可以使用**延迟+转发**技术解决，最好是在**程序编译时进行优化**，通过调整指令顺序避免出现该现象
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240502154939.png)

#### （3）控制冒险
- **定义**：当流水线遇到转移指令和其他改变 PC 值的指令而造成断流时，会引起**控制相关**
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240502155758.png)
**解决办法**
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240502160930.png)
- 每个时钟周期内可**并发多条独立指令**
- 并不影响流水线功能段的处理时间
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240502161143.png)
- 需要**多个处理部件**

#### （3）超流水线技术
- **定义**：在一个时钟周期内再分段
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240502161507.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240502171452.png)

![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240502171515.png)

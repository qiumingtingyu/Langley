...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240602152701.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240602171054.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240602163906.png)

![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240602163731.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240602164525.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240602165701.png)
...
- **缺点**：实现复杂，需要专门的通道硬件支持
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240602170531.png)


### 3、I/O 软件层次结构
- 将系统中的设备管理模块分为若干个层次，**每层都利用其下层提供的服务**，完成输入/输功能中的某些子功能，并**屏蔽这些功能的实现细节**，向高层提供服务
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240604145317.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240604152911.png)
...
- M时间：数据从缓冲区 ---> 用户
- C 时间：CPU 对一块数据处理的时间
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240604162046.png)
- **注意**：
	- 当缓冲区数据非空时，不能往缓冲区冲入数据，只能从缓冲区把数据传出
	- 当缓冲区为空时，可以往缓冲区冲入数据，但必须把缓冲区充满以后，才能从缓冲区把数据传出
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240604162941.png)
- **结论**：采用单缓冲策略，处理一块数据平均耗时 **Max (C, T)+M**

##### 2）双缓冲
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240604163543.png)
- 假设初始状态为：工作区空，其中一个缓冲区满，另一个缓冲区空
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240604163708.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240604164039.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240604164403.png)
- **收容输入**：
...
#### （3）高速缓存与缓冲区的对比
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240604165750.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240605151539.png)

##### 1）设备控制表（DCT）
- 系统为每个设备配置一张 DCT，用于记录设备情况
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240605151817.png)

##### 2）控制器控制表（COCT）
- 每个设备控制器对应一张 COCT
- OS 根据 COCT 的信息对控制器进行操作和管理
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240605151934.png)

##### 3）通道控制表（CHCT）
- 每个通道对应一张 CHCT
- OS 根据 CHCT 的信息对通道进行操作和管理
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240605152039.png)

##### 4）系统设备表（SDT）
- 记录了**系统中全部设备**的情况
- 每个设备对应一个表目
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240605152145.png)
...
- **逻辑设备表（LUT）**：
	...
	- 两种方式：
		...
		2. **为每个用户设置一张 LUT**：
			- 同时在多用户系统中都配置系统设备表
			- 不同用户可以使用相同的逻辑设备名
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240606152840.png)
...
- **引入目的**：
	...
	- 实现预输入、缓输出
- **组成**：外围控制机 + 更高速的设备（磁带）
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240606154710.png)


#### （2）假脱机技术的实现
- **假脱机技术**：又叫 SPOOLing 技术，用**软件**的方式模拟脱机技术，不需要外围机，是一项将**独占设备改造成共享设备**的软件技术
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240606154731.png)
##### 1）输入井和输出井
- 在磁盘上开辟出的两个存储区域
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240606161932.png)
...
- 如何使所有的设备驱动程序都有统一的接口：
	...
	- 对设备进行保护
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240608154956.png)
...
- 每个扇区的数据量相同（如 1 KB）
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240608162835.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240608165227.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240608200246.png)
...
![](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240608200246.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240608200254.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240608200301.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240608201651.png)
- **思考**：磁盘的物理地址是（柱面号，盘面号，扇区号）而不是（盘面号，柱面号，扇区号）
- 读取地址连续的磁盘块时，采用（柱面号，盘面号，扇区号）的地址结构可以**减少磁头移动消耗的时间**
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240608201940.png)

##### 2）错位命名
- **思想**：让相邻盘面的扇区编号错位
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240608202223.png)

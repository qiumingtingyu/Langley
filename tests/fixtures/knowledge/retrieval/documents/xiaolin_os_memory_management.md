...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514104221.png)
1. **编译**：由编译程序将用户源代码编译成若干目标模块
2. **链接**：由链接程序将编译后形成的一组目标模块，以及它们所需的库函数链接在一起，形成一个完整的装入地址，有三种方式：
	- **静态链接**：在程序运行之前，先将各目标模块及它们所需的库函数连接成一个完整的可执行文件（装入模块），之后不再拆开
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514104926.png)
	- **装入时动态链接**：将各目标模块装入内存时，边装入边链接的链接方式
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514105049.png)
	- **运行时动态链接**：在程序执行中需要改目标模块时，才对它进行链接，优点时便于修改和更新，便于实现对目标模块的共享
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514105239.png)
3. **装入**：由装入程序将装入模块装入内存运行，有三种方式：
	- **绝对装入**：
		- 在编译时，如果知道程序将放到内存中的哪个位置，编译程序将产生绝对地址的目标代码装入程序按照装入模块中的地址，将程序和数据装入内存
		- 程序中的逻辑地址与实际内存地址完全相同
		- 只适用于**单道程序环境**
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514110017.png)
	- **可重定位装入**【静态重定位】：
		- 编译、链接后的**装入模块的地址都是从0开始**的，指令中使用的地址、数据存放的地址都是相对于起始地址而言的逻辑地址
		- 装入时对地址进行“重定位”，将逻辑地址变换为物理地址【地址变换是在装入时一次完成的】
		- 装入时，必须给作业分配所要求的全部内存空间
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514110246.png)
	- **动态运行时装入**【动态重定位】：
		- 编译、链接后的**装入模块的地址都是从 0 开始**的
		- 装入程序把装入模块装入内存后，并不会立即把逻辑地址转换为物理地址，而是把**地址转换推迟到程序真正要执行时才进行**
		- 因此装入内存后所有的地址依然是逻辑地址
		- 这种方式需要一个**重定位寄存器**的支持
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514110614.png)
#### （2）逻辑地址与物理地址
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240819144242.png)
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240819144324.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514113902.png)
#### （5）内存共享
- 只有**只读**区域的进程空间可以共享
...
- **缺点**：
	...
	- **有内部碎片**
	- 存储器的利用率极低
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514120842.png)
##### 2）固定分区分配
- **定义**：将用户内存空间大小划分若干固定大小的分区，每个分区只装入一道作业
- **分区大小相等**：缺乏灵活性，但是很适合用于用一台计算机控制多个相同对象的场合
- **分区大小不等**：增加了灵活性，可以满足不同大小的进程需求，根据常在系统中运行的作业大小情况进行划分
- 为了方便分配与回收，建立一张**分区使用表**，每个表项对应一个分区，包括分区大小、起始地址及状态
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514121528.png)
- **优点**：实现简单，**无外部碎片**
- **缺点**：
	- 程序太大可能放不下任何一个分区
...
- 页表的作用是实现从**页号到物理块号的地址映射**
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514195528.png)
- 计算：每个页表项占多少字节？
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514201010.png)
##### 3）地址结构
- 页号 + 页内偏移量
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514194129.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514202615.png)
...
- **快表（TLB）**【相联存储器】：
	...
	- 基于局部性原理
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514205038.png)
1. 计算页号、页内偏移量
2. 检查页号合法性
3. **查快表**，若找到匹配的页号，直接读出对应的物理块号，**一次访存**
4. 若没有找到，访问主存的页表，读出页表项后，同时将其存入快表，**两次访存**
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514205530.png)
##### 6）两级页表
- **逻辑地址结构**：一级页号 + 二级页号 + 页内偏移量
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514210155.png)
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514210216.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240514212622.png)
...
- 段表用于实现从逻辑段到物理内存区的映射
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240516141903.png)
##### 3）地址结构
- 段号 + 段内偏移量
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240516142157.png)
...
- **段表寄存器**：
	- 存放段表始址 F 和段表长度 M
	- 存放于进程的 PCB 中
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240516142535.png)
- 地址变换过程：
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240516150206.png)
##### 2）地址结构
- 段号 + 页号 + 页内偏移量
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240516150145.png)
- 段号的位数决定了**每个进程最多可以分几个段**
- 页号位数决定了**每个段最大有多少页**
...
![](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240516151437.png)
- 地址空间是**二维**的
- **三次访存**：
	- 第一次：查段表
	- 第二次：查页表
...
- **注意**：
	...
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240516155342.png)
#### （4）虚拟内存的实现
- **方式** 【离散分配】：
	- 请求分页存储管理
	- 请求分段存储管理
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240517160356.png)
...
- 相比基本分页管理，增加的步骤：
	...
	- 需要修改请求页表中新增的表项
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240517162050.png)
...
4. 根据形成的物理地址访存
![](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240517162123.png)
**注意**：
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240517170214.png)
- **固定分配局部置换**：
...
1. **系统拥有足够的对换区空间**：
	...
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240518135710.png)
2. **系统缺少足够的对换区空间**：
	- 凡是**不会被修改的数据都直接从文件区调入**，由于这些页面不会被修改，因此换出时不必写回磁盘，下次需要时再从文件区调入即可
	- 对于可能**被修改的部分**，换出时需写回磁盘对换区，下次需要时再从对换区调入
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240518135502.png)
3. **UNIX 方式**：
	- **运行之前**进程有关的数据**全部放在文件区**，故未使用过的页面，都可从文件区调入
	- 若被使用过的页面需要换出，则写回对换区，下次需要时从对换区调入
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240518135621.png)
#### （5）如何调入页面
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240518141818.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240518142907.png)
...
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240518143420.png)
...
- **特点**：
	...
	- 实现困难，开销大
	- 算法性能好
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240518144133.png)
...
- **特点**：
	...
	- 实现简单
	- 未考虑页面是否被修改
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240518144856.png)
![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240518144911.png)
...
- **特性**：
	...
	- 多个进程可以映射一个文件，方便共享
	![image.png](https://cdn.jsdelivr.net/gh/LYuYang61/Notes-Images@main/408/20240518152815.png)

...
- `LANGLEY_WORKING_CONTEXT_BUDGET_ESTIMATE` (defaults to `16000`)
- `LANGLEY_CONVERSATION_COMPACTION_TRIGGER_ESTIMATE` (defaults to `12000`)
- `LANGLEY_RECENT_RAW_TARGET_ESTIMATE` (defaults to `6000`)
- `LANGLEY_COMPACT_STATE_TARGET_ESTIMATE` (defaults to `2000`)
- `LANGLEY_CONVERSATION_COMPACTOR_MODEL` (defaults to the answer model baseline)
- `LANGLEY_MEMORY_ESTIMATED_TOKEN_BUDGET` (defaults to `8192`)
- `LANGLEY_MEMORY_POLICY_MODEL` (optional; the calibrated baseline is `qwen3.7-plus-2026-05-26`)
- `LANGLEY_MEMORY_POLICY_ESTIMATED_TOKEN_BUDGET` (required with the policy model; `24576` is the calibrated V1 Load-All write capacity for that model and current MemoryPolicy framing, not the provider context-window limit; recalibrate after changing the model, prompt, or framing)
